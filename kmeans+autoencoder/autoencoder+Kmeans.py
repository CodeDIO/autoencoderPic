# -*- coding: utf-8 -*-
"""
Created on Wed Apr 22 10:27:31 2020
...
input_n=16384*3       # 输入的数据是16384个，即（128*128）维度的
hidden1_n=500
hidden2_n=400
leastd = int(np.sqrt(hidden2_n))
hidden3_n=500
output_n=16384*3      # 输入的数据也是16384个，即（128*128）维度的
...
num = 128
img = Image.open('pictures/狗子.png')
img = img.resize((num,num))
pixel = np.array(img)
pixel = pixel.reshape((num*num , 3))/255
pixel = pixel.reshape(pixel.shape[0]*pixel.shape[1],-1)
...
with tf.Session() as sess:
    ...
    encode_decode = encode_decode.T.reshape(16384,3).reshape(128,128,3)
    hidden = hidden.T.reshape(20,20)
plt.subplot(131)  
plt.imshow(encode_decode)  
plt.subplot(132)  
plt.imshow(hidden) 
# ============================================================================
# Kmeans部分
def setCenters(X, k):
    m = X.shape  # 获取数据数量及特征数
    # 根据数据信息以及簇个数构建各簇中心点
    centroids = np.zeros((k))
    # 在数据中随机找k个点作为簇中心点
    idx = np.random.randint(0, m[0], k)
...
def setNewCenter(X, idx, k):
    # 得到矩阵大小，初始化矩阵
    centroids = np.zeros((k))  # 即数据又k簇，每簇的中心点由数据的特征数决定
    # 计算聚类中心
    for i in range(k):
        # 获取数据中类别为i的数据索引
        ...
        if (len(indices[0])==0):
            break
        centroids[i] = (np.sum(X[indices[0]], axis=0) / len(indices[0])).ravel()
    return centroids
...
A = hidden
X = np.reshape(A,(A.shape[0] * A.shape[1],1))
# 随机初始化聚类中心
initial_centroids = setCenters(X, 2)  # 随机抽取2个样本点作为聚类中心
# 运行kmeans聚类算法，迭代10次
...
with tf.Session() as sess:
    ...
    saver = tf.train.Saver()
    saver.restore(sess, "outmodel/out_model")
    X_recovered = X_recovered+10
    X_recovered = X_recovered.reshape(hidden2_n,-1).T
